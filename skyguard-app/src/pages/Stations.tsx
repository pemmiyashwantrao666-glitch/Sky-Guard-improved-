...
import { Search, SlidersHorizontal, MapPin, Gauge, Wifi, WifiOff, Wrench } from "lucide-react";
...
export function Stations() {
  ...
  const getMaintenanceDate = (stationId: string) => {
    ...
    return task ? new Date(task.dueDate).toLocaleDateString() : "—";
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="p-4">
          ...
          <p className="mt-1 text-2xl font-bold text-ink-navy">{total}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-graphite/60 uppercase tracking-wide">Online</p>
          <p className="mt-1 text-2xl font-bold text-healthy-green">{online}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-graphite/60 uppercase tracking-wide">Offline</p>
          <p className="mt-1 text-2xl font-bold text-alert-coral">{offline}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-graphite/60 uppercase tracking-wide">Maintenance</p>
          <p className="mt-1 text-2xl font-bold text-signal-amber">{maintenance}</p>
        </Card>
      </div>
                      ...
                      <div className="grid gap-4 p-4 lg:grid-cols-[minmax(180px,1.4fr)_minmax(145px,1fr)_110px_75px_125px_90px_90px] lg:items-center lg:gap-5">
                        ...
                        <div>
                          <span className="text-xs text-graphite/60">
                            {new Date(station.lastSync).toLocaleString([], {
                              hour: "2-digit",
                              minute: "2-digit",
                              day: "2-digit",
                              month: "short",
