...
function SensorNetworkGlobe() {
  ...
  useEffect(() => {
    ...
    let frame = 0;
    const animate = () => { frame = requestAnimationFrame(animate); if (!inView) return; globe.rotation.y += 0.0018; points.forEach((point, index) => { const pulse = 1 + Math.sin(Date.now() * 0.003 + index) * 0.18; point.scale.setScalar(pulse); }); renderer.render(scene, camera); };
    animate();
  ...
  return <div className="sensor-globe"><div ref={mountRef} className="sensor-globe-canvas" /><div className="globe-caption"><span className="globe-pulse" /> LIVE SENSOR NETWORK <b>128 NODES</b></div><div className="globe-legend"><span><i className="legend-cyan" /> Healthy</span><span><i className="legend-amber" /> Watch</span><span><i className="legend-red" /> Anomaly</span></div></div>;
}
...
export function Landing() {
  ...
  return <main className="landing-page">
    <nav className="landing-nav"><a className="wordmark" href="/"><img src="/skyguard-mark.svg" alt="" className="wordmark-mark" /> SkyGuard</a><div className="nav-links"><a href="#platform">Platform</a><a href="#workflow">How it works</a><a href="#use-cases">Use cases</a><a href="#contact">Contact</a></div><button className="nav-action" onClick={() => navigate("/login")}>Access Console <ArrowUpRight size={13} /></button></nav>
